...
static void fuzz_sink(void *ctx, const TimuiEvent *ev){ (void)ctx; (void)ev; }
...
TIMUI_TEST(test_fuzz_parser_random_stream){
    ...
    timui_input_feed(&p, buf, sizeof buf, fuzz_sink, 0);
    TIMUI_CHECK(1);              /* reached here -> no crash */
}
...
TIMUI_TEST(test_fuzz_parser_adversarial){
    ...
    timui_input_init(&p);
    timui_input_feed(&p, adv, sizeof adv, fuzz_sink, 0);
    TIMUI_CHECK(1);              /* no crash */
}
